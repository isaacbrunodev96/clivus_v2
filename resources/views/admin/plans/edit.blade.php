...
@section('title', 'Editar Plano - CLIVUS')
...
<div class="max-w-2xl mx-auto">
    <div class="rounded-xl p-6" style="background-color: rgb(var(--card)); border: 1px solid rgb(var(--border)); box-shadow: var(--shadow);">
        <h2 class="text-2xl font-bold mb-6">Editar Plano: {{ $plan->name }}</h2>
        ...
        <form action="{{ route('admin.plans.update', $plan) }}" method="POST" class="space-y-6">
            ...
            <div class="grid grid-cols-1 md:grid-cols-2 gap-6">
                ...
                <div>
                    ...
                    <input type="text" id="price" name="price" value="{{ old('price', $plan->price) }}" required
                        class="w-full px-4 py-2 rounded-lg border transition-colors focus:outline-none focus:ring-2 money-mask"
                        style="background-color: rgb(var(--bg)); border-color: rgb(var(--border)); color: rgb(var(--text)); focus:ring-color: rgb(var(--primary));">
                </div>
                ...
                <div class="md:col-span-2">
                    ...
                    <textarea id="description" name="description" rows="3"
                        ...
                        style="background-color: rgb(var(--bg)); border-color: rgb(var(--border)); color: rgb(var(--text)); focus:ring-color: rgb(var(--primary));">{{ old('description', $plan->description) }}</textarea>
                </div>
                
                <div>
                    <label for="max_accounts" class="block text-sm font-medium mb-2">Máx. Contas Bancárias</label>
                    <input type="number" id="max_accounts" name="max_accounts" value="{{ old('max_accounts', $plan->max_accounts) }}" min="0"
                        class="w-full px-4 py-2 rounded-lg border transition-colors focus:outline-none focus:ring-2"
                        style="background-color: rgb(var(--bg)); border-color: rgb(var(--border)); color: rgb(var(--text)); focus:ring-color: rgb(var(--primary));">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="max_companies" name="max_companies" value="{{ old('max_companies', $plan->max_companies) }}" min="1"
                        class="w-full px-4 py-2 rounded-lg border transition-colors focus:outline-none focus:ring-2"
                        style="background-color: rgb(var(--bg)); border-color: rgb(var(--border)); color: rgb(var(--text)); focus:ring-color: rgb(var(--primary));">
                </div>
                ...
                <div>
                    ...
                    <input type="number" id="max_transactions_per_month" name="max_transactions_per_month" value="{{ old('max_transactions_per_month', $plan->max_transactions_per_month) }}" min="0"
                        class="w-full px-4 py-2 rounded-lg border transition-colors focus:outline-none focus:ring-2"
                        style="background-color: rgb(var(--bg)); border-color: rgb(var(--border)); color: rgb(var(--text)); focus:ring-color: rgb(var(--primary));">
                </div>
                        ...
                        <div class="mb-4">
                            ...
                            <div class="space-y-2">
                                ...
                                <label class="flex items-center p-2 rounded-lg hover:bg-opacity-50 transition-colors" style="background-color: rgba(var(--primary), 0.05);">
                                    <input type="checkbox" name="allowed_modules[]" value="{{ $module->slug }}" {{ in_array($module->slug, $selectedModules) ? 'checked' : '' }}
                                        class="rounded border-gray-300" style="border-color: rgb(var(--border));">
